"""Retrieving sensor versions by serial and hostnames.
Requires FalconPy v0.8.6+
"""
import argparse
...
LIMIT = 1000     # Quick limit to prove pagination
while OFFSET < TOTAL:
    OFFSET, TOTAL, devices = device_list(OFFSET, LIMIT, SORT)
    details = device_detail(devices)
    for detail in details:
        DISPLAYED += 1
        print(f"{detail['serial_number'], detail['hostname']}")
